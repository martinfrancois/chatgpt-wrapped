...
def line_chart(items, title, color="#00AAFF", y_axis_label="Count"):
    ...
    for i in range(0, len(items), step):
        parts.append(f'<text x="{x(i):.1f}" y="{height - 16}" text-anchor="middle" class="axis-label">{esc(items[i][0])}</text>')
    parts.append(f'<text x="{left + chart_w / 2:.1f}" y="{height - 44}" text-anchor="middle" class="axis-note">Month</text>')
...
def build_html(data):
    ...
    cards = "".join([
        ...
        kpi("ChatGPT replies", human_int(visible_assistant_messages), f"{human_int(data['reasoning_total'])} reasoning traces kept separate"),
